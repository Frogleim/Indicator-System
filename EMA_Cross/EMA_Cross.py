...
async def fetch_ohlcv(symbol, timeframe="15m", limit=1000):
    exchange = ccxt_async.binance({
        'options': {
            'defaultType': 'future',  # Specify the Futures market
        },
    })
...
async def calculate_indicators(symbol):
    try:
        ...
        if df.empty:
            loggs.error_logs_logger.error(f'DataFrame is empty after fetching data for {symbol}')
            return df
        ...
        if df.empty:
            loggs.error_logs_logger.error(f'DataFrame is empty after calculations for {symbol}')

        return df
    except Exception as e:
        loggs.error_logs_logger.error(f'Error in calculate_indicators for {symbol}: {e}')
        return pd.DataFrame()
...
async def main():
    try:
        ...
        loggs.error_logs_logger.error(f'EMA Crossover script down!\nError message: {e}')
